...
df_grouped = df_with_date.groupBy("Post Date", "Subreddit").agg(
    ...
    sum("token_count").alias("total_token_count"),
    avg("token_count").alias("avg_token_count")
)
...
df_with_month = df_with_date.withColumn("Month", date_format(df_with_date["Post Date"], "yyyy-MM"))
