...
df_with_token_count = df_with_token_count.withColumnRenamed("Post Date", "Post_Date") \
...
def query_model(text):
    API_URL = "https://rizi5ypveik24is8.us-east-1.aws.endpoints.huggingface.cloud"
    headers = {
...
df_with_predictions.write.format("delta").mode("overwrite").save("/mnt/gold/reddit/post_predictions_llama31")
display(df_with_predictions)

# COMMAND ----------

df_with_predictions = spark.read.format("delta").load("/mnt/gold/reddit/post_predictions_llama31")
display(df_with_predictions)
